...
impl Mesh {
	pub fn gen_cube(_sk: &impl StereoKitContext, size: impl Into<MVec3>, subdivisions: i32) -> Result<Self> {
		let size = size.into();
		Ok(Mesh {
			mesh: NonNull::new(unsafe {
				stereokit_sys::mesh_gen_cube(vec3_from(size), subdivisions)
			}).ok_or(Report::msg(format!("Failed to create a cube from the size '{:?}' and subdivisions '{}'.", size, subdivisions)))?,
		})
	}
	pub fn gen_plane(
		...
		let (dimensions, plane_normal, plane_top_direction) = (dimensions.into(), plane_normal.into(), plane_top_direction.into());
		Ok(Mesh {
			mesh: NonNull::new(unsafe {
