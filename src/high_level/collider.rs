...
impl Collider {
	pub fn get_type(&self) -> ColliderType {
		match self {
			Collider::CapsuleCollider(_) => ColliderType::CapsuleCollider,
		}
	}
}
...
impl CapsuleCollider {
	pub fn new(
		point1: impl Into<values::MVec3>,
		point2: impl Into<values::MVec3>,
		radius: f32,
	) -> Self {
		Self {
	...
	pub fn from(sk: &impl StereoKitContext, model: &Model) -> Self {
		...
		let trans_rot_mat = Mat4::from_rotation_translation(
			model.get_matrix().to_scale_rotation_translation().1,
			model.get_matrix().to_scale_rotation_translation().2,
		);
