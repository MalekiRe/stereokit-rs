use crate::values::{vec3_from, MVec3, vec3_to, IntegerType};
	...
	fn play_sound(&self, position: impl Into<MVec3>, volume: f32) -> SoundInstance {
		SoundInstance {
			sound_instance: unsafe {
				stereokit_sys::sound_play(self.get_sound_ptr().as_ptr(), vec3_from(position.into()), volume)
			},
		}
	}
}
...
impl SoundStream {
	pub fn create(buffer_duration: f32) -> Self {
		Self {
			sound_t: NonNull::new(unsafe {
			stereokit_sys::sound_create_stream(buffer_duration)
			}).unwrap()
		}
	}
	pub fn write_samples(&self, in_arr_samples: &[f32]) {
		unsafe {
			stereokit_sys::sound_write_samples(self.sound_t.as_ptr(), in_arr_samples.as_ptr(), in_arr_samples.len() as u64);
		}
	}
	pub fn read_samples(&self, out_arr_samples: &mut [f32]) -> u64 {
		unsafe {
			stereokit_sys::sound_read_samples(self.sound_t.as_ptr(), out_arr_samples.as_mut_ptr(), out_arr_samples.len() as u64)
		}
	}
	pub fn unread_samples(&self) -> u64 {
		unsafe {
			stereokit_sys::sound_unread_samples(self.sound_t.as_ptr())
		}
	}
}
