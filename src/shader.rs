...
impl Shader {
	pub fn from_file(sk: &impl StereoKitContext, file_path: impl AsRef<Path>) -> Result<Self> {
		let file_path = ustr(file_path.as_ref().as_os_str().to_str().ok_or(Report::msg("file to string conversion failed"))?);
		Ok(Shader {
			shader: NonNull::new(unsafe {
				stereokit_sys::shader_create_file(file_path.as_char_ptr())
			}).ok_or(Report::msg(format!("The Shader from file '{}' could not be loaded", file_path)))?,
		})
	}
	...
	pub fn from_name(sk: &impl StereoKitContext, name: &str) -> Result<Self> {
		Ok(Shader {
			shader: NonNull::new(unsafe { shader_find(ustr(name).as_char_ptr()) }).ok_or(Report::msg(format!("The Shader '{}' could not be located from it's name", name))).suggestion("Try something in the format of 'default/shader_pbr'")?,
		})
	}
}
