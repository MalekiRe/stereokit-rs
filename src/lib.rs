...
impl From<sk_settings_t> for Settings {
    fn from(value: sk_settings_t) -> Self {
        match value {
            sk_settings_t { app_name, assets_folder, display_preference, blend_preference, no_flatscreen_fallback, depth_mode, log_filter, overlay_app, overlay_priority, flatscreen_pos_x, flatscreen_pos_y, flatscreen_width, flatscreen_height, disable_flatscreen_mr_sim, disable_desktop_input_window, disable_unfocused_sleep, render_scaling, render_multisample: _, android_java_vm: _, android_activity: _ } => {
                unsafe {
                    Self {
                        app_name: CStr::from_ptr(app_name).to_str().unwrap().to_string(),
                        assets_folder: CStr::from_ptr(assets_folder).to_str().unwrap().to_string().parse().unwrap(),
...
impl Into<sk_settings_t> for Settings {
    fn into(self) -> sk_settings_t {
        match self {
            Settings { app_name, assets_folder, display_preference, no_flatscreen_fallback, blend_preference, depth_mode, log_filter, overlay_app, overlay_priority, flatscreen_pos_x, flatscreen_pos_y, flatscreen_width, flatscreen_height, display_flatscreen_mr_sim, disable_desktop_input_window, disable_unfocused_sleep, render_scaling } => {
...
impl SettingsBuilder {
    ...
    fn build(self) -> Settings {
        match self {
            SettingsBuilder { app_name, assets_folder, display_preference, no_flatscreen_fallback, blend_preference, depth_mode, log_filter, overlay_app, overlay_priority, flatscreen_pos_x, flatscreen_pos_y, flatscreen_width, flatscreen_height, display_flatscreen_mr_sim, disable_desktop_input_window, disable_unfocused_sleep, render_scaling } => {
                Settings {
                    app_name,
                    assets_folder,
                    display_preference,
...
impl From<system_info_t> for SystemInfo {
    fn from(value: system_info_t) -> Self {
        match value {
            system_info_t { display_type, display_width, display_height, spatial_bridge_present, perception_bridge_present, eye_tracking_present, overlay_app, world_occlusion_present, world_raycast_present } => {
                SystemInfo {
...
impl Pose {
    const IDENTITY: Pose = Pose { position: Vec3::new(0.0, 0.0, 0.0), orientation: Quat::IDENTITY };
...
impl Into<hand_joint_t> for HandJoint {
    fn into(self) -> hand_joint_t {
        match self {
            HandJoint { position, orientation, radius } => {
                hand_joint_t {
                    position: position.into(),
                    orientation: quat {
                        x: orientation.x,
...
impl From<hand_joint_t> for HandJoint {
    fn from(value: hand_joint_t) -> Self {
        match value {
            hand_joint_t { position, orientation, radius } => {
                Self {
                    position: position.into(),
                    orientation: Quat::from_xyzw(orientation.x, orientation.y, orientation.z, orientation.w),
                    radius,
...
impl Into<hand_t> for Hand {
    fn into(self) -> hand_t {
        match self {
            Hand { fingers, wrist, palm, pinch_pt, handedness, tracked_state, pinch_state, grip_state, size, pinch_activation, grip_activation } => {
                hand_t {
...
impl Into<controller_t> for Controller {
    fn into(self) -> controller_t {
        match self {
            Controller { pose, palm, aim, tracked, tracked_pos, tracked_rot, stick_click, x1, x2, trigger, grip, stick } => {
                controller_t {
                    pose: pose.into(),
                    palm: palm.into(),
                    aim: aim.into(),
...
pub trait CStereoKitDraw: CStereoKitSingleThread {
    ...
    fn line_addv(
        ...
        unsafe {
            stereokit_sys::line_addv(start.into(), end.into())
        }
    }
    ...
    fn render_global_texture(&self, register_slot: i32, texture: impl AsRef<Tex>) {
        unsafe {
            stereokit_sys::render_global_texture(register_slot, texture.as_ref().0.as_ptr())
        }
    }
    ...
    unsafe fn render_get_device(&self, device: *mut *mut std::os::raw::c_void, context: *mut *mut std::os::raw::c_void) {
...
pub trait CStereoKitSingleThread: CStereoKitMultiThread {
    ...
    fn hierarchy_push(&self, transform: impl Into<Mat4>) {
        let transform = transform.into().into();
        unsafe {
            stereokit_sys::hierarchy_push(&transform)
        }
    }
    ...
    fn hierarchy_set_enabled(&self, enabled: bool) {
        unsafe {
            stereokit_sys::hierarchy_set_enabled(enabled as bool32_t)
        }
    }
    ...
    fn hierarchy_is_enabled(&self) -> bool {
        unsafe {
            stereokit_sys::hierarchy_is_enabled() != 0
        }
    }
    ...
    fn hierarchy_to_local_point(&self, world_point: impl Into<Vec3>) -> Vec3 {
        ...
        unsafe {
            stereokit_sys::hierarchy_to_local_point(&world_point)
        }.into()
    }
    ...
    fn hierarchy_to_local_direction(&self, world_dir: impl Into<Vec3>) -> Vec3 {
        ...
        unsafe {
            stereokit_sys::hierarchy_to_local_direction(&world_dir)
        }.into()
    }
    ...
    fn hierarchy_to_local_rotation(&self, world_orientation: impl Into<Quat>) -> Quat {
        ...
        let q2 = unsafe {
            stereokit_sys::hierarchy_to_local_rotation(&quat)
        };
        Quat::from_xyzw(
            q2.x, q2.y, q2.z, q2.w
        )
    ...
    fn hierarchy_to_local_pose(&self, world_pose: Pose) -> Pose {
        ...
        unsafe {
            stereokit_sys::hierarchy_to_local_pose(&pose)
        }.into()
    }
    ...
    fn hierarchy_to_world_point(&self, local_pt: impl Into<Vec3>) -> Vec3 {
        ...
        unsafe {
            stereokit_sys::hierarchy_to_world_point(&local_pt)
        }.into()
    }
    ...
    fn hierarchy_to_world_direction(&self, local_dir: impl Into<Vec3>) -> Vec3 {
        ...
        unsafe {
            stereokit_sys::hierarchy_to_world_direction(&local_dir)
        }.into()
    }
    ...
    fn hierarchy_to_world_rotation(&self, local_orientation: impl Into<Quat>) -> Quat {
        ...
        let q2 = unsafe {
            stereokit_sys::hierarchy_to_world_rotation(&l1)
        };
    ...
    fn hierarchy_to_world_pose(&self, local_pose: Pose) -> Pose {
        ...
        unsafe {
            stereokit_sys::hierarchy_to_world_pose(&pose)
        }.into()
    }
}
pub trait CStereoKitMultiThread {
    ...
    fn active_display_mode(&self) -> DisplayMode {
        unsafe {
            stereokit_sys::sk_active_display_mode()
        }.into()
    }
    ...
    fn version_name(&self) -> &str {
        unsafe {
            CStr::from_ptr(stereokit_sys::sk_version_name()).to_str().unwrap()
        }
    }
    ...
    fn device_display_get_type(&self) -> DisplayType {
        unsafe {
            stereokit_sys::device_display_get_type()
        }.into()
    }
    ...
    fn device_display_get_blend(&self) -> DisplayBlend {
        unsafe {
            stereokit_sys::device_display_get_blend()
        }.into()
    }
    ...
    fn device_display_set_blend(
        &self,
        blend: DisplayBlend
    ) -> bool {
        unsafe {
            stereokit_sys::device_display_set_blend(blend.into()) != 0
        }
    }

    /// Tells if a particular blend mode is valid on this device.
    /// Some devices may be capable of more then one blend mode.
    fn device_display_valid_blend(
        &self,
        blend: DisplayBlend
    ) -> bool {
        unsafe {
            stereokit_sys::device_display_valid_blend(blend.into()) != 0
        }
    }
    ...
    fn device_display_get_refresh_rate(&self) -> f32 {
        unsafe {
            stereokit_sys::device_display_get_refresh_rate()
        }
    }
    ...
    fn device_display_get_width(&self) -> i32 {
        unsafe {
            stereokit_sys::device_display_get_width()
        }
    }

    fn device_display_get_height(&self)-> i32 {
        unsafe {
            stereokit_sys::device_display_get_height()
        }
    }

    fn device_display_get_fov(&self) -> FovInfo {
        unsafe {
            stereokit_sys::device_display_get_fov()
        }
    }
    ...
    fn device_get_tracking(&self) -> DeviceTracking {
        unsafe {
            stereokit_sys::device_get_tracking()
        }.into()
    }
    ...
    fn device_get_name(&self) -> &str {
        unsafe {
            CStr::from_ptr(stereokit_sys::device_get_name()).to_str().unwrap()
        }
    }
    ...
    fn device_get_gpu(&self) -> &str {
        unsafe {
            CStr::from_ptr(stereokit_sys::device_get_gpu()).to_str().unwrap()
        }
    }
    ...
    fn device_has_eye_gaze(&self) -> bool {
        unsafe {
            stereokit_sys::device_has_eye_gaze() != 0
        }
    }
    ...
    fn device_has_hand_tracking(&self) -> bool {
        unsafe {
            stereokit_sys::device_has_hand_tracking() != 0
        }
    }

    fn time_get_raw(&self) -> f64 {
        unsafe {
            stereokit_sys::time_get_raw()
        }
    }

    fn time_get_f32_unscaled(&self) -> f32 {
        unsafe {
            stereokit_sys::time_getf_unscaled()
        }
    }

    fn time_get_unscaled(&self) -> f64 {
        unsafe {
            stereokit_sys::time_get_unscaled()
        }
    }

    fn time_get_f32(&self) -> f32 {
        unsafe {
            stereokit_sys::time_getf()
        }
    }

    fn time_get(&self) -> f64 {
        unsafe {
            stereokit_sys::time_get()
        }
    }

    fn time_elapsed_f32_unscaled(&self) -> f32 {
        unsafe {
            stereokit_sys::time_elapsedf_unscaled()
        }
    }

    fn time_elapsed_unscaled(&self) -> f64 {
        unsafe {
            stereokit_sys::time_elapsed_unscaled()
        }
    }

    fn time_total_raw(&self) -> f64 {
        unsafe {
            stereokit_sys::time_total_raw()
        }
    }

    fn time_total_f32_unscaled(&self) -> f32 {
        unsafe {
            stereokit_sys::time_totalf_unscaled()
        }
    }

    fn time_total_unscaled(&self) -> f64 {
        unsafe {
            stereokit_sys::time_total_unscaled()
        }
    }

    fn time_total_f32(&self) -> f32 {
        unsafe {
            stereokit_sys::time_totalf()
        }
    }

    fn time_total(&self) -> f64 {
        unsafe {
            stereokit_sys::time_total()
        }
    }

    fn time_step_f32_unscaled(&self) -> f32 {
        unsafe {
            stereokit_sys::time_stepf_unscaled()
        }
    }

    fn time_step_unscaled(&self) -> f64 {
        unsafe {
            stereokit_sys::time_step_unscaled()
        }
    }

    fn time_step_f32(&self) -> f32 {
        unsafe {
            stereokit_sys::time_stepf()
        }
    }

    fn time_step(&self) -> f64 {
        unsafe {
            stereokit_sys::time_step()
        }
    }
    ...
    fn time_scale(
        ...
        unsafe {
            stereokit_sys::time_scale(scale)
        }
    }
    ...
    fn time_set_time(
        ...
        unsafe {
            stereokit_sys::time_set_time(total_seconds, frame_elapsed_seconds)
        }
    }
    ...
    fn mesh_find<S: Into<String> + Clone>(&self, name: S) -> SkResult<Mesh> {
        let c_str = std::ffi::CString::new(name.clone().into()).map_err(|_| {
            StereoKitError::MeshCString(name.clone().into())
        })?;
        Ok(Mesh(NonNull::new(unsafe {
            stereokit_sys::mesh_find(c_str.as_ptr())
        }).ok_or(
            StereoKitError::MeshFind(name.into())
        )?))
    }
    ...
    fn mesh_copy<Me: AsRef<Mesh>>(&self, mesh: Me) -> Mesh {
        Mesh(NonNull::new(unsafe {
            stereokit_sys::mesh_copy(mesh.as_ref().0.as_ptr())
        }).unwrap())
    }
    ...
    fn mesh_set_id<Me: AsRef<Mesh>, S: Into<String>>(&self, mesh: Me, id: S) {
        ...
        unsafe {
            stereokit_sys::mesh_set_id(mesh.as_ref().0.as_ptr(), c_str.as_ptr())
        }
    }

    fn mesh_get_id<Me: AsRef<Mesh>>(&self, mesh: Me) -> &str {
        unsafe {
            CStr::from_ptr(stereokit_sys::mesh_get_id(mesh.as_ref().0.as_ptr()))
        }.to_str().unwrap()
    }
    ...
    fn mesh_addref<Me: AsRef<Mesh>>(&self, mesh: Me) {
        unsafe {
            stereokit_sys::mesh_addref(mesh.as_ref().0.as_ptr())
        }
    }
    ...
    fn mesh_set_keep_data<Me: AsRef<Mesh>>(&self, mesh: Me, keep_data: bool) {
        unsafe {
            stereokit_sys::mesh_set_keep_data(mesh.as_ref().0.as_ptr(), keep_data as stereokit_sys::bool32_t);
        }
    }

    fn mesh_get_keep_data<Me: AsRef<Mesh>>(&self, mesh: Me) -> bool {
        unsafe {
            stereokit_sys::mesh_get_keep_data(mesh.as_ref().0.as_ptr()) != 0
        }
    }

    fn mesh_set_data<Me: AsRef<Mesh>>(&self, mesh: Me, vertices: &[Vert], arrays: &[u32], calculate_bounds: bool) {
        unsafe {
            stereokit_sys::mesh_set_data(mesh.as_ref().0.as_ptr(), std::mem::transmute(vertices.as_ptr()), vertices.len() as i32, std::mem::transmute(arrays.as_ptr()), arrays.len() as i32, calculate_bounds as bool32_t)
        }
    }
    ...
    fn mesh_set_verts<Me: AsRef<Mesh>>(&self, mesh: Me, vertices: &[Vert], calculate_bounds: bool) {
        unsafe {
            stereokit_sys::mesh_set_verts(mesh.as_ref().0.as_ptr(), std::mem::transmute(vertices.as_ptr()), vertices.len() as i32, calculate_bounds as bool32_t)
        }
    }
    ...
    fn mesh_get_verts_copy<Me: AsRef<Mesh>>(&self, mesh: Me) -> Vec<Vert> {
        unsafe {
            let mut verts_pointer = null_mut();
            let mut verts_len = 0;
            stereokit_sys::mesh_get_verts(mesh.as_ref().0.as_ptr(), &mut verts_pointer, &mut verts_len, 0);
            Vec::from_raw_parts(std::mem::transmute(verts_pointer), verts_len as usize, verts_len as usize)
        }
    }
    ...
    fn mesh_set_inds(&self, mesh: &Mesh, inds: &[i32]) {
        unsafe {
            stereokit_sys::mesh_set_inds(mesh.0.as_ptr(), std::mem::transmute(inds.as_ptr()), inds.len() as i32)
        }
    }
    ...
    fn mesh_get_inds_copy<Me: AsRef<Mesh>>(&self, mesh: Me) -> Vec<i32> {
        unsafe {
            ...
            Vec::from_raw_parts(std::mem::transmute(inds_ptr), inds_len as usize, inds_len as usize)
        }
    }
    ...
    fn mesh_get_ind_count<Me: AsRef<Mesh>>(&self, mesh: Me) -> i32 {
        unsafe {
            stereokit_sys::mesh_get_ind_count(mesh.as_ref().0.as_ptr())
        }
    }
    ...
    fn mesh_set_bounds<Me: AsRef<Mesh>>(&self, mesh: Me, bounds: impl AsRef<Bounds>) {
        unsafe {
            stereokit_sys::mesh_set_bounds(mesh.as_ref().0.as_ptr(), std::mem::transmute(bounds.as_ref() as *const Bounds));
        }
    }
    ...
    fn mesh_get_bounds<Me: AsRef<Mesh>>(&self, mesh: Me) -> Bounds {
        unsafe {
            stereokit_sys::mesh_get_bounds(mesh.as_ref().0.as_ptr())
        }.into()
    }

    fn mesh_has_skin<Me: AsRef<Mesh>>(&self, mesh: Me) -> bool {
        unsafe {
            stereokit_sys::mesh_has_skin(mesh.as_ref().0.as_ptr()) != 0
        }
    }
    ...
    fn mesh_set_skin<Me: AsRef<Mesh>>(
        ...
        let bone_resting_transforms = bone_resting_transforms.into_iter().map(|a| a.clone().into()).collect::<Vec<_>>();
        unsafe {
    ...
    fn mesh_update_skin<Me: AsRef<Mesh>>(
        &self,
        mesh: Me,
        bone_transforms: &[Mat4],
    ) {
        let bone_transforms = bone_transforms.iter().map(|a| a.clone().into()).collect::<Vec<_>>();
        unsafe {
    ...
    fn mesh_gen_cube(
        &self,
        dimensions: impl Into<Vec3>,
        subdivisions: i32,
    ) -> Mesh {
        unsafe {
            stereokit_sys::mesh_gen_cube(
                dimensions.into().into(),
                subdivisions
            )
        }.into()
    }
    ...
    fn mesh_gen_rounded_cube(
        ...
        unsafe {
            stereokit_sys::mesh_gen_rounded_cube(
                dimensions.into(),
                edge_radius,
                subdivisions
            )
    ...
    fn mesh_gen_cylinder(
        ...
        unsafe {
            stereokit_sys::mesh_gen_cylinder(
                diameter,
                depth,
                direction.into(),
                subdivisions
    ...
    fn tex_find<S: Into<String> + Clone>(&self, id: S) -> SkResult<Tex> {
        let c_str = CString::new(id.clone().into()).map_err(|_| {
            StereoKitError::TexCString(id.clone().into())
        })?;

        Ok(Tex(NonNull::new(unsafe {
            stereokit_sys::tex_find(c_str.as_ptr())
        }).ok_or(
            StereoKitError::TexFind(id.clone().into())
        )?))
    }
    ...
    fn tex_create_file(
        ...
        let c_str = CString::new(path.to_str().ok_or(StereoKitError::TexFile(path_buf.clone(), "CString conversion".to_string()))?).map_err(|_| StereoKitError::TexFile(path_buf.clone(), "CString Conversion".to_string()))?;
        Ok(Tex(NonNull::new(
            unsafe {
                stereokit_sys::tex_create_file(
                    c_str.as_ptr(),
                    srgb_data as bool32_t,
                    priority
                )
            ...
            StereoKitError::TexFile(path_buf.clone(), "tex_create_file failed".to_string())
        )?))
    }
    ...
    fn tex_create_cubemap_file(
        ...
        Ok((unsafe {
            std::mem::transmute(*spherical_harmonic_ptr)
        }, tex))
    }
    ...
    fn tex_set_id<T: AsRef<Tex>, S: Into<String> + Clone>(&self, tex: T, id: S) {
        ...
        unsafe {
            stereokit_sys::tex_set_id(tex.as_ref().0.as_ptr(), c_str.as_ptr())
        }
    }
    ...
    fn tex_get_id<T: AsRef<Tex>>(&self, tex: T) -> &str {
        unsafe {
            CStr::from_ptr(stereokit_sys::tex_get_id(tex.as_ref().0.as_ptr())).to_str().unwrap()
        }
    }
    ...
    unsafe fn tex_addref<T: AsRef<Tex>>(&self, tex: T) {
        unsafe {
            stereokit_sys::tex_addref(tex.as_ref().0.as_ptr())
        }
    }
    ...
    fn tex_asset_state<T: AsRef<Tex>>(&self, tex: T) -> AssetState {
        unsafe {
            std::mem::transmute(stereokit_sys::tex_asset_state(tex.as_ref().0.as_ptr()))
        }
    }
    ...
    fn tex_get_format<T: AsRef<Tex>>(&self, tex: T) -> TextureFormat {
        unsafe {
            std::mem::transmute(stereokit_sys::tex_get_format(tex.as_ref().0.as_ptr()))
        }
    }
    ...
    fn tex_get_width<T: AsRef<Tex>>(&self, tex: T) -> i32 {
        unsafe {
            stereokit_sys::tex_get_width(tex.as_ref().0.as_ptr())
        }
    }
    ...
    fn tex_get_height<T: AsRef<Tex>>(&self, tex: T) -> i32 {
        unsafe {
            stereokit_sys::tex_get_height(tex.as_ref().0.as_ptr())
        }
    }
    ...
    fn tex_set_sample<T: AsRef<Tex>>(&self, tex: T, sample: TextureSample) {
        unsafe {
            stereokit_sys::tex_set_sample(tex.as_ref().0.as_ptr(), sample as tex_sample_)
        }
    }
    ...
    fn tex_get_sample<T: AsRef<Tex>>(&self, tex: T) -> TextureSample {
        unsafe {
            std::mem::transmute(stereokit_sys::tex_get_sample(tex.as_ref().0.as_ptr()))
        }
    }
    ...
    fn tex_get_address<T: AsRef<Tex>>(&self, tex: T) -> TextureAddress {
        unsafe {
            std::mem::transmute(stereokit_sys::tex_get_address(tex.as_ref().0.as_ptr()))
        }
    }
    ...
    fn tex_set_anisotropy<T: AsRef<Tex>>(&self, tex: T, anisotropy_level: i32) {
        unsafe {
            stereokit_sys::tex_set_anisotropy(tex.as_ref().0.as_ptr(), anisotropy_level)
        }
    }
    ...
    fn tex_get_anisotropy<T: AsRef<Tex>>(&self, tex: T) -> i32 {
        unsafe {
            stereokit_sys::tex_get_anisotropy(tex.as_ref().0.as_ptr())
        }
    }

    fn tex_get_mips<T: AsRef<Tex>>(&self, tex: T) -> i32 {
        unsafe {
            stereokit_sys::tex_get_mips(tex.as_ref().0.as_ptr())
        }
    }
    ...
    fn tex_set_loading_fallback<T: AsRef<Tex>>(&self, tex: T) {
        unsafe {
            stereokit_sys::tex_set_loading_fallback(tex.as_ref().0.as_ptr())
        }
    }
    ...
    fn tex_set_error_fallback<T: AsRef<Tex>>(&self, error_texture: T) {
        unsafe {
            stereokit_sys::tex_set_error_fallback(error_texture.as_ref().0.as_ptr())
        }
    }
    ...
    fn tex_get_cubemap_lighting<T: AsRef<Tex>>(&self, cubemap_texture: T) -> SphericalHarmonics {
        unsafe {
            std::mem::transmute(stereokit_sys::tex_get_cubemap_lighting(cubemap_texture.as_ref().0.as_ptr()))
        }
    }
    ...
    fn font_find<S: Into<String> + Clone>(&self, id: S) -> SkResult<Font> {
        ...
        Ok(Font(NonNull::new(unsafe {
            stereokit_sys::font_find(c_str.as_ptr())
        }).ok_or(
    ...
    fn font_create(
        ...
        let path = file_utf8.as_ref();
        let path_buf = path.to_path_buf();
        let c_str = CString::new(path_buf.clone().to_str().ok_or(
            StereoKitError::FontFile(path_buf.clone(), "CString conversion".to_string())
        )?).map_err(|_| StereoKitError::FontFile(path_buf.clone(), "CString conversion".to_string()))?;
        Ok(Font(NonNull::new(unsafe {
            stereokit_sys::font_create(c_str.as_ptr())
        }).ok_or(
            StereoKitError::FontFile(path_buf.clone(), "font_create failed".to_string())
        )?))
    }

    fn font_set_id<S: Into<String> + Clone>(
        &self,
        ...
        let c_str = CString::new(id.into()).unwrap();
        unsafe {
            stereokit_sys::font_set_id(font.as_ref().0.as_ptr(), c_str.as_ptr())
        }
    }

    fn font_get_id(&self, font: impl AsRef<Font>) -> &str {
        unsafe {
            CStr::from_ptr(stereokit_sys::font_get_id(font.as_ref().0.as_ptr()))
        }.to_str().unwrap()
    }

    unsafe fn font_addref(&self, font: impl AsRef<Font>) {
        unsafe {
            stereokit_sys::font_addref(font.as_ref().0.as_ptr())
        }
    }
    ...
    fn shader_find<S: Into<String> + Clone>(&self, id: S) -> SkResult<Shader>{
        let c_str = CString::new(id.clone().into())
            .map_err(|_| StereoKitError::ShaderFind(id.clone().into(), "CString conversion".to_string()))?;
        Ok(Shader(NonNull::new(unsafe {
            stereokit_sys::shader_find(c_str.as_ptr())
        }).ok_or(StereoKitError::ShaderFind(id.clone().into(), "shader_find failed".to_string()))?))
    }
    ...
    fn shader_create_file(&self, file: impl AsRef<Path>) -> SkResult<Shader> {
        ...
        let c_str = CString::new(
            path_buf.clone().to_str().ok_or(
                StereoKitError::ShaderFile(path_buf.clone(), "CString conversion".to_string())
            )?
        ).map_err(|_| StereoKitError::ShaderFile(path_buf.clone(), "CString conversion".to_string()))?;
        Ok(Shader(NonNull::new(unsafe {
            stereokit_sys::shader_create_file(c_str.as_ptr())
    ...
    fn shader_set_id<S: Into<String> + Clone>(&self, shader: impl AsRef<Shader>, id: S) {
        let c_str = CString::new(id.into()).unwrap();
        unsafe {
            stereokit_sys::shader_set_id(shader.as_ref().0.as_ptr(), c_str.as_ptr())
        }
    }
    ...
    fn shader_get_id(&self, shader: impl AsRef<Shader>) -> &str {
        unsafe {
            CStr::from_ptr(
                stereokit_sys::shader_get_id(shader.as_ref().0.as_ptr())
            )
        }.to_str().unwrap()
    }
    ...
    fn shader_get_name(&self, shader: impl AsRef<Shader>) -> &str {
        unsafe {
            CStr::from_ptr(
                stereokit_sys::shader_get_name(shader.as_ref().0.as_ptr())
            )
        }.to_str().unwrap()
    }
    ...
    unsafe fn shader_addref(&self, shader: impl AsRef<Shader>) {
        unsafe {
            stereokit_sys::shader_addref(shader.as_ref().0.as_ptr())
        }
    }
    ...
    fn material_find<S: Into<String> + Clone>(&self, id: S) -> SkResult<Material> {
        let c_str = CString::new(id.clone().into())
            .map_err(|_| StereoKitError::MaterialFind(id.clone().into(), "CString conversion".to_string()))?;
        Ok(Material(NonNull::new(unsafe {
            stereokit_sys::material_find(c_str.as_ptr())
        }).ok_or(StereoKitError::MaterialFind(id.clone().into(), "material_find failed".to_string()))?))
    }
    ...
    fn material_create(&self, shader: impl AsRef<Shader>) -> Material {
        Material(NonNull::new(unsafe {
            stereokit_sys::material_create(shader.as_ref().0.as_ptr())
        }).unwrap())
    }
    ...
    fn material_copy<M: AsRef<Material>>(&self, material: M) -> Material {
        Material(NonNull::new(unsafe {
            stereokit_sys::material_copy(material.as_ref().0.as_ptr())
        }).unwrap())
    }
    ...
    fn material_copy_id<S: Into<String> + Clone>(&self, id: S) -> Material {
        let c_str = CString::new(id.into()).unwrap();
        Material(NonNull::new(unsafe {
            stereokit_sys::material_copy_id(c_str.as_ptr())
        }).unwrap())
    }
    ...
    fn material_set_id<M: AsRef<Material>, S: Into<String> + Clone>(&self, material: M, id: S) {
        let c_str = CString::new(id.into()).unwrap();
        unsafe {
            stereokit_sys::material_set_id(material.as_ref().0.as_ptr(), c_str.as_ptr())
        }
    }

    fn material_get_id<M: AsRef<Material>>(&self, material: M) -> &str {
        unsafe {
            CStr::from_ptr(stereokit_sys::material_get_id(material.as_ref().0.as_ptr()))
        }.to_str().unwrap()
    }

    unsafe fn material_addref<M: AsRef<Material>>(&self, material: M) {
        unsafe {
            stereokit_sys::material_addref(material.as_ref().0.as_ptr())
        }
    }
    ...
    fn material_set_transparency<M: AsRef<Material>>(&self, material: M, mode: Transparency) {
        unsafe {
            stereokit_sys::material_set_transparency(material.as_ref().0.as_ptr(), mode as transparency_);
        }
    }
    ...
    fn material_set_cull<M: AsRef<Material>>(&self, material: M, mode: CullMode) {
        unsafe {
            stereokit_sys::material_set_cull(material.as_ref().0.as_ptr(), mode as cull_)
        }
    }
    ...
    fn material_set_wireframe<M: AsRef<Material>>(&self, material: M, wireframe: bool) {
        unsafe {
            stereokit_sys::material_set_wireframe(material.as_ref().0.as_ptr(), wireframe as bool32_t)
        }
    }
    ...
    fn material_set_depth_test<M: AsRef<Material>>(&self, material: M, depth_test_mode: DepthTest) {
        unsafe {
            stereokit_sys::material_set_depth_test(material.as_ref().0.as_ptr(), depth_test_mode as depth_test_)
        }
    }
    ...
    fn material_set_depth_write<M: AsRef<Material>>(&self, material: M, write_enabled: bool) {
        unsafe {
            stereokit_sys::material_set_depth_write(material.as_ref().0.as_ptr(), write_enabled as bool32_t)
        }
    }
    ...
    fn material_set_queue_offset<M: AsRef<Material>>(&self, material: M, offset: i32) {
        unsafe {
            stereokit_sys::material_set_queue_offset(material.as_ref().0.as_ptr(), offset)
        }
    }

    fn material_set_chain<M: AsRef<Material>>(&self, material: M, chain_material: M) {
        unsafe {
            stereokit_sys::material_set_chain(material.as_ref().0.as_ptr(), chain_material.as_ref().0.as_ptr())
        }
    }
    ...
    fn material_get_transparency<M: AsRef<Material>>(&self, material: M) -> Transparency {
        unsafe {
            std::mem::transmute(stereokit_sys::material_get_transparency(material.as_ref().0.as_ptr()))
        }
    }
    ...
    fn material_get_cull<M: AsRef<Material>>(&self, material: M) -> CullMode {
        unsafe {
            std::mem::transmute(stereokit_sys::material_get_cull(material.as_ref().0.as_ptr()))
        }
    }
    ...
    fn material_get_wireframe<M: AsRef<Material>>(&self, material: M) -> bool {
        unsafe {
            stereokit_sys::material_get_wireframe(material.as_ref().0.as_ptr()) != 0
        }
    }
    ...
    fn material_get_depth_test<M: AsRef<Material>>(&self, material: M) -> DepthTest {
        unsafe {
            std::mem::transmute(stereokit_sys::material_get_depth_test(material.as_ref().0.as_ptr()))
        }
    }
    ...
    fn material_get_depth_write<M: AsRef<Material>>(&self, material: M) -> bool {
        unsafe {
            stereokit_sys::material_get_depth_write(material.as_ref().0.as_ptr()) != 0
        }
    }
    ...
    fn material_get_queue_offset<M: AsRef<Material>>(&self, material: M) -> i32 {
        unsafe {
            stereokit_sys::material_get_queue_offset(material.as_ref().0.as_ptr())
        }
    }
    ...
    fn material_set_float<M: AsRef<Material>, S: AsRef<str>>(
        ...
        unsafe {
            stereokit_sys::material_set_float(
                material.as_ref().0.as_ptr(),
                c_str.as_ptr(),
                value
            )
    ...
    fn material_set_color<M: AsRef<Material>, S: AsRef<str>>(
        ...
        let c_str = CString::new(name.as_ref()).unwrap();
        unsafe {
            stereokit_sys::material_set_color(
                material.as_ref().0.as_ptr(),
                c_str.as_ptr(),
                value
            )
    ...
    fn material_set_int<M: AsRef<Material>, S: AsRef<str>>(
        ...
        let c_str = CString::new(name.as_ref()).unwrap();
        unsafe {
            stereokit_sys::material_set_int(
                material.as_ref().0.as_ptr(),
                c_str.as_ptr(),
                value
            )
    ...
    fn material_set_uint<M: AsRef<Material>, S: AsRef<str>>(
        ...
        let c_str = CString::new(name.as_ref()).unwrap();
        unsafe {
            stereokit_sys::material_set_uint(
                material.as_ref().0.as_ptr(),
                c_str.as_ptr(),
                value
            )
    ...
    fn material_get_float<M: AsRef<Material>, S: AsRef<str>>(
        &self,
        material: M,
        name: S,
    ) -> f32 {
        let c_str = CString::new(name.as_ref()).unwrap();
        unsafe {
            stereokit_sys::material_get_float(
                material.as_ref().0.as_ptr(),
                c_str.as_ptr()
            )
        }
    ...
    fn material_get_vector2<M: AsRef<Material>, S: AsRef<str>>(
        ...
        let c_str = CString::new(name.as_ref()).unwrap();
        unsafe {
            stereokit_sys::material_get_vector2(material.as_ref().0.as_ptr(), c_str.as_ptr())
        }.into()
    }
    ...
    fn material_get_vector3<M: AsRef<Material>, S: AsRef<str>>(
        ...
        let c_str = CString::new(name.as_ref()).unwrap();
        unsafe {
            stereokit_sys::material_get_vector3(material.as_ref().0.as_ptr(), c_str.as_ptr())
        }.into()
    }
    ...
    fn material_get_vector4<M: AsRef<Material>, S: AsRef<str>>(
        ...
        let c_str = CString::new(name.as_ref()).unwrap();
        unsafe {
            stereokit_sys::material_get_vector4(material.as_ref().0.as_ptr(), c_str.as_ptr())
        }.into()
    }

    fn material_get_int<M: AsRef<Material>, S: AsRef<str>>(
        &self,
        material: M,
        name: S,
    ) -> i32 {
        let c_str = CString::new(name.as_ref()).unwrap();
        unsafe {
            stereokit_sys::material_get_int(material.as_ref().0.as_ptr(), c_str.as_ptr())
        }
    }

    fn material_get_bool<M: AsRef<Material>, S: AsRef<str>>(
        &self,
        material: M,
        name: S,
    ) -> bool {
    ...
    fn material_get_uint<M: AsRef<Material>, S: AsRef<str>>(
        &self,
        material: M,
        name: S,
    ) -> u32 {
        let c_str = CString::new(name.as_ref()).unwrap();
        unsafe {
            stereokit_sys::material_get_uint(material.as_ref().0.as_ptr(), c_str.as_ptr())
        }
    }

    fn material_get_matrix<M: AsRef<Material>, S: AsRef<str>>(
        &self,
        material: M,
        name: S,
    ) -> Mat4 {
        let c_str = CString::new(name.as_ref()).unwrap();
        unsafe {
            stereokit_sys::material_get_matrix(material.as_ref().0.as_ptr(), c_str.as_ptr())
        }.into()
    }

    fn material_get_texture<M: AsRef<Material>, S: AsRef<str>>(
        &self,
        material: M,
        name: S,
    ) -> Tex {
    ...
    fn material_get_param_count<M: AsRef<Material>>(&self, material: M) -> i32 {
        unsafe {
            stereokit_sys::material_get_param_count(material.as_ref().0.as_ptr())
        }
    }
    ...
    fn material_buffer_create(&self, register_slot: i32, size: i32) -> MaterialBuffer {
        MaterialBuffer(NonNull::new(unsafe {
            stereokit_sys::material_buffer_create(register_slot, size)
        }).unwrap())
    }

    unsafe fn material_buffer_set_data<MB: AsRef<MaterialBuffer>>(&self, material_buffer: MB, buffer_data: *const std::os::raw::c_void) {
    ...
    unsafe fn text_make_style_mat<F: AsRef<Font>, M: AsRef<Material>>(
        ...
        TextStyle(
            stereokit_sys::text_make_style_mat(
                font.as_ref().0.as_ptr(),
                character_height,
                material.as_ref().0.as_ptr(),
                color_gamma
    ...
    fn text_size<S: AsRef<str>>(&self, text_utf8: S, style: TextStyle) -> Vec2 {
        let text_utf8 = CString::new(text_utf8.as_ref()).unwrap();
        unsafe {
            stereokit_sys::text_size(text_utf8.as_ptr(), style.0)
        }.into()
    }
    ...
    fn text_style_get_material(&self, style: TextStyle) -> Material {
        Material(NonNull::new(unsafe {
            stereokit_sys::text_style_get_material(style.0)
        }).unwrap())
    }

    fn text_style_get_char_height(&self, style: TextStyle) -> f32 {
        unsafe {
            stereokit_sys::text_style_get_char_height(style.0)
        }
    }

    /// Looks for a Model asset that’s already loaded, matching the given id!
    fn model_find<S: Into<String> + Clone>(
        &self,
        id: S
    ) -> SkResult<Model> {
        let str = std::ffi::CString::new(id.clone().into()).map_err(|_| {
            StereoKitError::ModelFile(id.clone().into())
        })?;
        Ok(Model(NonNull::new(unsafe {
            stereokit_sys::model_find(str.as_ptr())
        }).ok_or(
            StereoKitError::ModelFile(id.clone().into())
        )?))
    }
    ...
    fn model_copy<M: AsRef<Model>>(
        &self,
        model: M
    ) -> Model {
        Model(NonNull::new(unsafe {
            stereokit_sys::model_copy(model.as_ref().0.as_ptr())
        }).unwrap())
    }

    fn model_create(&self) -> Model {
        Model(NonNull::new(unsafe {
            stereokit_sys::model_create()
        }).unwrap())
    }
    ...
    fn model_create_mesh<Me: AsRef<Mesh>, Ma: AsRef<Material>>(
        ...
        Model(NonNull::new(unsafe {
            stereokit_sys::model_create_mesh(mesh.as_ref().0.as_ptr(), material.as_ref().0.as_ptr())
        }).unwrap())
    }
    ...
    fn model_create_mem<S: Into<String> + Clone>(
        ...
        let c_file_name = std::ffi::CString::new(file_name.clone().into()).map_err(|_| {
            StereoKitError::ModelFromMem(file_name.clone().into(), String::from("file name is not a valid CString"))
        })?;
        Ok(Model(NonNull::new(unsafe {
            stereokit_sys::model_create_mem(
                ...
                memory.len(),
                shader.map(|shader| shader.0.as_ptr())
                    .unwrap_or(null_mut())
            )
        }).ok_or(StereoKitError::ModelFromMem(file_name.clone().into(), String::from("model_create_mem failed")))?))
    }
    ...
    fn model_create_file(
        ...
        let c_str = CString::new(filename.as_ref()).map_err(|_| {
            StereoKitError::ModelFromFile(filename.as_ref().to_string(), "CString conversion".to_string())
        })?;
        Ok(Model(NonNull::new(unsafe {
            ...
            StereoKitError::ModelFromFile(filename.as_ref().to_string(), "model_create_file failed".to_string())
        )?))
    }
    ...
    fn model_get_id<M: AsRef<Model>>(&self, model: M) -> &str {
        unsafe {
            CStr::from_ptr(
                stereokit_sys::model_get_id(model.as_ref().0.as_ptr())
            )
        }.to_str().unwrap()
    }
    ...
    fn model_recalculate_bounds<M: AsRef<Model>>(&self, model: M) {
        unsafe {
            stereokit_sys::model_recalculate_bounds(model.as_ref().0.as_ptr())
        }
    }
    ...
    fn model_set_bounds<M: AsRef<Model>>(&self, model: M, bounds: impl AsRef<Bounds>) {
        ...
        unsafe {
            stereokit_sys::model_set_bounds(model.as_ref().0.as_ptr(), std::mem::transmute(bounds as *const Bounds))
        }
    }
    ...
    fn model_step_anim<M: AsRef<Model>>(&self, model: M) {
        unsafe {
            stereokit_sys::model_step_anim(model.as_ref().0.as_ptr())
        }
    }
    ...
    fn model_play_anim_idx<M: AsRef<Model>>(
        &self,
        model: M,
        index: i32,
        mode: AnimMode,
        ...
        unsafe {
            stereokit_sys::model_play_anim_idx(
                model.as_ref().0.as_ptr(),
                index,
                mode as anim_mode_
            )
    ...
    fn model_set_anim_time<M: AsRef<Model>>(&self, model: M, time: f32) {
        unsafe {
            stereokit_sys::model_set_anim_time(
                model.as_ref().0.as_ptr(),
                time
            )
        }
    ...
    fn model_set_anim_completion<M: AsRef<Model>>(&self, model: M, percent: f32) {
        unsafe {
            stereokit_sys::model_set_anim_completion(model.as_ref().0.as_ptr(), percent)
        }
    }
    ...
    fn model_anim_count<M: AsRef<Model>>(&self, model: M) -> i32 {
        unsafe {
            stereokit_sys::model_anim_count(model.as_ref().0.as_ptr())
        }
    }

    fn model_anim_active<M: AsRef<Model>>(&self, model: M) -> i32 {
        unsafe {
            stereokit_sys::model_anim_active(model.as_ref().0.as_ptr())
        }
    }

    fn model_anim_active_mode<M: AsRef<Model>>(&self, model: M) -> AnimMode {
        unsafe {
            std::mem::transmute(stereokit_sys::model_anim_active_mode(model.as_ref().0.as_ptr()))
        }
    }

    fn model_anim_active_time<M: AsRef<Model>>(&self, model: M) -> f32 {
        unsafe {
            stereokit_sys::model_anim_active_time(model.as_ref().0.as_ptr())
        }
    }

    fn model_anim_active_completion<M: AsRef<Model>>(&self, model: M) -> f32 {
        unsafe {
            stereokit_sys::model_anim_active_completion(model.as_ref().0.as_ptr())
        }
    }

    fn model_anim_get_name<M: AsRef<Model>>(&self, model: M, index: i32) -> Option<&str> {
        unsafe {
            CStr::from_ptr(stereokit_sys::model_anim_get_name(model.as_ref().0.as_ptr(), index))
        }.to_str().map(|a| Some(a)).unwrap_or(None)
    }

    fn model_anim_get_duration<M: AsRef<Model>>(&self, model: M, index: i32) -> f32 {
        unsafe {
            stereokit_sys::model_anim_get_duration(model.as_ref().0.as_ptr(), index)
        }
    }
    ...
    fn model_get_name<M: AsRef<Model>>(&self, model: M, subset: i32) -> &str {
        unsafe {
            CStr::from_ptr(stereokit_sys::model_get_name(model.as_ref().0.as_ptr(), subset))
        }.to_str().unwrap()
    }
    ...
    fn model_get_transform<M: AsRef<Model>>(&self, model: M, subset: i32) -> Mat4 {
        unsafe {
            stereokit_sys::model_get_transform(model.as_ref().0.as_ptr(), subset)
        }.into()
    }

    /// Changes the Material for the subset to a new one!
    fn model_set_material<M: AsRef<Model>>(&self, model: M, subset: i32, material: impl AsRef<Material>) {
        unsafe {
    ...
    fn model_set_transform<M: AsRef<Model>>(&self, model: M, subset: i32, transform: impl Into<Mat4>) {
        let transform = transform.into().into();
        unsafe {
            stereokit_sys::model_set_transform(model.as_ref().0.as_ptr(), subset, &transform)
        }
    }
    ...
    fn model_remove_subset<M: AsRef<Model>>(&self, model: M, subset: i32) {
        unsafe {
            stereokit_sys::model_remove_subset(model.as_ref().0.as_ptr(), subset)
        }
    }
    ...
    fn model_subset_count<M: AsRef<Model>>(&self, model: M) -> i32 {
        unsafe {
            stereokit_sys::model_subset_count(model.as_ref().0.as_ptr())
        }
    }
    ...
    fn model_node_sibling<M: AsRef<Model>>(
        ...
        match unsafe {
            stereokit_sys::model_node_sibling(
                model.as_ref().0.as_ptr(),
                node
            )
        } { -1 => None,
    ...
    fn model_node_parent<M: AsRef<Model>>(
        ...
        match unsafe {
            stereokit_sys::model_node_parent(
                model.as_ref().0.as_ptr(),
                node
            )
        } {
    ...
    fn model_node_child<M: AsRef<Model>>(
        ...
        match unsafe {
            stereokit_sys::model_node_child(
                model.as_ref().0.as_ptr(),
                node,
            )
        } {
    ...
    fn model_node_count<M: AsRef<Model>>(&self, model: M) -> i32 {
        unsafe {
            stereokit_sys::model_node_count(model.as_ref().0.as_ptr())
        }
    }

    fn model_node_index<M: AsRef<Model>>(
        &self,
        model: M,
        index: i32,
    ) -> Option<ModelNodeId> {
        match unsafe {
            stereokit_sys::model_node_index(
                model.as_ref().0.as_ptr(),
                index
            )
        } {
    ...
    fn model_node_visual_count<M: AsRef<Model>>(&self, model: M) -> i32 {
        unsafe {
            stereokit_sys::model_node_visual_count(model.as_ref().0.as_ptr())
        }
    }
    ...
    fn model_node_iterate<M: AsRef<Model>>(
        ...
        match unsafe {
            stereokit_sys::model_node_iterate(
                model.as_ref().0.as_ptr(),
                node
            )
        } {
    ...
    fn model_node_get_root<M: AsRef<Model>>(&self, model: M) -> ModelNodeId {
        unsafe {
            stereokit_sys::model_node_get_root(model.as_ref().0.as_ptr())
        }
    }

    fn model_node_get_name<M: AsRef<Model>>(
        &self,
        model: M,
        node: ModelNodeId
    ) -> Option<&str> {
        unsafe {
            CStr::from_ptr(stereokit_sys::model_node_get_name(
                model.as_ref().0.as_ptr(),
                node
            )).to_str().map(|s| Some(s)).unwrap_or(None)
        }
    }

    fn model_node_get_visible<M: AsRef<Model>>(
        &self,
        model: M,
        node: ModelNodeId,
    ) -> bool {
        unsafe {
            stereokit_sys::model_node_get_visible(
                model.as_ref().0.as_ptr(),
                node
            ) != 0
        }
    ...
    fn model_node_get_material<M: AsRef<Model>>(
        ...
        Some(Material(NonNull::new(unsafe {
            stereokit_sys::model_node_get_material(
                model.as_ref().0.as_ptr(),
                node
            )
        })?))
    ...
    fn model_node_get_mesh<M: AsRef<Model>>(
        &self,
        model: M,
        node: ModelNodeId,
    ) -> Option<Mesh> {
        Some(Mesh(NonNull::new(unsafe {
            stereokit_sys::model_node_get_mesh(
                model.as_ref().0.as_ptr(),
                node
            )
        })?))
    ...
    fn model_node_get_transform_model<M: AsRef<Model>>(
        &self,
        model: M,
        node: ModelNodeId,
    ) -> Mat4 {
        unsafe {
            stereokit_sys::model_node_get_transform_model(
                model.as_ref().0.as_ptr(),
                node
            )
        }.into()
    ...
    fn model_node_get_transform_local<M: AsRef<Model>>(
        &self,
        model: M,
        node: ModelNodeId,
    ) -> Mat4 {
        unsafe {
            stereokit_sys::model_node_get_transform_local(
                model.as_ref().0.as_ptr(),
                node
            )
        }.into()
    ...
    fn model_node_set_name<M: AsRef<Model>, S: AsRef<str>>(
        ...
        let name = CString::new(name.as_ref()).unwrap();
        unsafe {
            stereokit_sys::model_node_set_name(
                model.as_ref().0.as_ptr(),
                node,
                name.as_ptr()
            )
    ...
    fn model_node_set_visible<M: AsRef<Model>>(
        &self,
        model: M,
        node: ModelNodeId,
        visible: bool
    ...
    fn model_node_info_clear<M: AsRef<Model>>(
        &self,
        model: M,
        node: ModelNodeId,
    ) {
        unsafe {
            stereokit_sys::model_node_info_clear(
                model.as_ref().0.as_ptr(),
                node
            )
        }
    ...
    fn model_node_info_count<M: AsRef<Model>>(
        &self,
        model: M,
        node: ModelNodeId
    ) -> ModelNodeId {
        unsafe {
            stereokit_sys::model_node_info_count(
                model.as_ref().0.as_ptr(),
                node
            )
        }
    ...
    fn sprite_find<S: AsRef<str>>(&self, id: S) -> SkResult<Sprite> {
        ...
        Ok(Sprite(NonNull::new(unsafe {
            stereokit_sys::sprite_find(cstr_id.as_ptr())
        }).ok_or(
            StereoKitError::SpriteFind(id.as_ref().to_string(), "sprite_find failed".to_string())
        )?))
    }
    ...
    fn sprite_get_id(&self, sprite: impl AsRef<Sprite>) -> Option<&str> {
        unsafe {
            CStr::from_ptr(stereokit_sys::sprite_get_id(sprite.as_ref().0.as_ptr()))
        }.to_str().map(|s| Some(s)).unwrap_or(None)
    }
    ...
    fn sprite_get_aspect(&self, sprite: impl AsRef<Sprite>) -> f32 {
        unsafe {
            stereokit_sys::sprite_get_aspect(sprite.as_ref().0.as_ptr())
        }
    }

    fn sprite_get_width(&self, sprite: impl AsRef<Sprite>) -> i32 {
        unsafe {
            stereokit_sys::sprite_get_width(sprite.as_ref().0.as_ptr())
        }
    }

    fn sprite_get_height(&self, sprite: impl AsRef<Sprite>) -> i32 {
        unsafe {
            stereokit_sys::sprite_get_height(sprite.as_ref().0.as_ptr())
        }
    }

    fn sprite_get_dimensions_normalized(&self, sprite: impl AsRef<Sprite>) -> Vec2 {
        unsafe {
            stereokit_sys::sprite_get_dimensions_normalized(sprite.as_ref().0.as_ptr())
        }.into()
    }
    ...
    fn render_set_clip(&self, near_plane: f32, far_plane: f32) {
        unsafe {
            stereokit_sys::render_set_clip(near_plane, far_plane)
        }
    }

    fn render_set_fov(&self, field_of_view_degrees: f32) {
        unsafe {
            stereokit_sys::render_set_fov(field_of_view_degrees)
        }
    }

    fn render_set_ortho_clip(&self, near_plane: f32, far_plane: f32) {
        unsafe {
            stereokit_sys::render_set_ortho_clip(near_plane, far_plane)
        }
    }

    fn render_set_ortho_size(&self, viewport_height_meters: f32) {
        unsafe {
            stereokit_sys::render_set_ortho_size(viewport_height_meters)
        }
    }

    fn render_set_projection(&self, proj: Projection) {
        unsafe {
            stereokit_sys::render_set_projection(proj.into())
        }
    }

    fn render_get_projection(&self) -> Projection {
        unsafe {
            std::mem::transmute(stereokit_sys::render_get_projection())
        }
    }
    ...
    fn render_set_cam_root(&self, cam_root: impl Into<Mat4>) {
        ...
        unsafe {
            stereokit_sys::render_set_cam_root(&cam_root)
        }
    }

    fn render_set_skytex(&self, sky_texture: impl AsRef<Tex>) {
        unsafe {
            stereokit_sys::render_set_skytex(sky_texture.as_ref().0.as_ptr())
        }
    }

    fn render_get_skylight(&self) -> SphericalHarmonics {
        unsafe {
            stereokit_sys::render_get_skylight()
        }.into()
    }
    ...
    fn render_set_scaling(&self, display_tex_scale: f32) {
        unsafe {
            stereokit_sys::render_set_scaling(display_tex_scale)
        }
    }

    fn render_get_scaling(&self) -> f32 {
        unsafe {
            stereokit_sys::render_get_scaling()
        }
    }

    fn render_set_multisample(&self, display_tex_multisample: i32) {
        unsafe {
            stereokit_sys::render_set_multisample(display_tex_multisample)
        }
    }

    fn render_get_multisample(&self) -> i32 {
        unsafe {
            stereokit_sys::render_get_multisample()
        }
    }
    ...
    fn render_has_capture_filter(&self) -> bool {
        unsafe {
            stereokit_sys::render_has_capture_filter() != 0
        }
    }

    fn render_set_clear_color(&self, color_gamma: Color128) {
        unsafe {
            stereokit_sys::render_set_clear_color(color_gamma)
        }
    }

    fn render_get_clear_color(&self) -> Color128 {
        unsafe {
            stereokit_sys::render_get_clear_color()
        }
    }

    fn render_enable_skytex(&self, show_sky: bool) {
        unsafe {
            stereokit_sys::render_enable_skytex(show_sky as bool32_t)
        }
    }

    fn render_enabled_skytex(&self) -> bool {
        unsafe {
            stereokit_sys::render_enabled_skytex() != 0
        }
    }

    fn sound_find<S: AsRef<str>>(&self, id: S) -> SkResult<Sound> {
        let c_id = CString::new(id.as_ref()).unwrap();
        Ok(Sound(NonNull::new(unsafe {
            stereokit_sys::sound_find(c_id.as_ptr())
        }).ok_or(
            StereoKitError::SoundFind(id.as_ref().to_string())
        )?))
    }
    ...
    fn sound_get_id(&self, sound: impl AsRef<Sound>) -> Option<&str> {
        unsafe {
            CStr::from_ptr(stereokit_sys::sound_get_id(sound.as_ref().0.as_ptr()))
        }.to_str().map(|s| Some(s)).unwrap_or(None)
    }
    ...
    fn sound_create<S: AsRef<Path>>(&self, filename: S) -> SkResult<Sound> {
        ...
        Ok(Sound(NonNull::new(unsafe {
            stereokit_sys::sound_create(c_name.as_ptr())
        }).ok_or(
            StereoKitError::SoundCreate(filename.as_ref().to_path_buf())
        )?))
    }

    fn sound_create_stream(&self, buffer: f32) -> Sound {
        Sound(NonNull::new(unsafe {
            stereokit_sys::sound_create_stream(buffer)
        }).unwrap())
    }

    fn sound_create_samples(&self, samples: &[f32]) -> Sound {
        Sound(NonNull::new(unsafe {
            stereokit_sys::sound_create_samples(
                samples.as_ptr(),
                samples.len() as u64
            )
        }).unwrap())
    ...
    fn sound_unread_samples(&self, sound: impl AsRef<Sound>) -> u64 {
        unsafe {
            stereokit_sys::sound_unread_samples(sound.as_ref().0.as_ptr())
        }
    }
    ...
    fn sound_cursor_samples(&self, sound: impl AsRef<Sound>) -> u64 {
        unsafe {
            stereokit_sys::sound_cursor_samples(sound.as_ref().0.as_ptr())
        }
    }

    fn sound_play(&self, sound: impl AsRef<Sound>, at: impl Into<Vec3>, volume: f32) -> SoundInstance {
        unsafe {
            stereokit_sys::sound_play(sound.as_ref().0.as_ptr(), at.into().into(), volume)
        }
    }

    fn sound_duration(&self, sound: impl AsRef<Sound>) -> f32 {
        unsafe {
            stereokit_sys::sound_duration(sound.as_ref().0.as_ptr())
        }
    }
    ...
    fn sound_inst_stop(&self, sound_instance: SoundInstance) {
        unsafe {
            stereokit_sys::sound_inst_stop(sound_instance)
        }
    }

    fn sound_inst_is_playing(&self, sound_instance: SoundInstance) -> bool {
        unsafe {
            stereokit_sys::sound_inst_is_playing(sound_instance) != 0
        }
    }

    fn sound_inst_set_pos(&self, sound_instance: SoundInstance, pos: impl Into<Vec3>) {
        unsafe {
            stereokit_sys::sound_inst_set_pos(sound_instance, pos.into().into())
        }
    }

    fn sound_inst_set_volume(&self, sound_instance: SoundInstance, volume: f32) {
        unsafe {
            stereokit_sys::sound_inst_set_volume(sound_instance, volume)
        }
    }

    fn sound_inst_get_volume(&self, sound_instance: SoundInstance) -> f32 {
        unsafe {
            stereokit_sys::sound_inst_get_volume(sound_instance)
        }
    }

    fn mic_device_count(&self) -> i32 {
        unsafe {
            stereokit_sys::mic_device_count()
        }
    }

    fn mic_device_name(&self, index: i32) -> &str {
        unsafe {
            CStr::from_ptr(stereokit_sys::mic_device_name(index))
        }.to_str().unwrap()
    }
    ...
    fn mic_start<S: AsRef<str>>(&self, device_name: S) -> bool {
        ...
        unsafe {
            stereokit_sys::mic_start(device_name.as_ptr()) != 0
        }
    }
    ...
    fn mic_get_stream(&self) -> Sound {
        Sound(NonNull::new(unsafe {
            stereokit_sys::mic_get_stream()
        }).unwrap())
    }

    fn mic_is_recording(&self) -> bool {
        unsafe {
            stereokit_sys::mic_is_recording() != 0
        }
    }
    ...
    fn input_hand(&self, hand: Handed) -> Hand {
        unsafe {
            *stereokit_sys::input_hand(hand as handed_)
        }.into()
    }
    ...
    fn input_controller(&self, hand: Handed) -> Controller {
        unsafe {
            *stereokit_sys::input_controller(hand as handed_)
        }.into()
    }
    ...
    fn input_head(&self) -> Pose {
        unsafe {
            *stereokit_sys::input_head()
        }.into()
    }

    fn input_eyes(&self) -> Pose {
        unsafe {
            *stereokit_sys::input_eyes()
        }.into()
    }

    fn input_mouse(&self) -> Mouse {
        unsafe {
            *stereokit_sys::input_mouse()
        }.into()
    }
    ...
    fn input_text_consume(&self) -> char {
        unsafe {
            char::from_u32(stereokit_sys::input_text_consume())
        }.unwrap()
    }

    fn input_text_reset(&self) {
        unsafe {
            stereokit_sys::input_text_reset()
        }
    }

    fn input_hand_visible(&self, hand: Handed, visible: bool) {
        unsafe {
            stereokit_sys::input_hand_visible(hand as handed_, visible as bool32_t)
        }
    }

    fn input_hand_material(&self, hand: Handed, material: Material) {
        unsafe {
            stereokit_sys::input_hand_material(hand as handed_, material.0.as_ptr())
        }
    }
    ...
    fn world_get_bounds_size(&self) -> Vec2 {
        unsafe {
            stereokit_sys::world_get_bounds_size()
        }.into()
    }
    ...
    fn world_raycast(&self, ray: Ray) -> Option<Ray> {
        ...
        match unsafe {
            stereokit_sys::world_raycast(ray.into(), ray_out) != 0
        } {
    ...
    fn world_set_occlusion_enabled(&self, enabled: bool) {
        unsafe {
            stereokit_sys::world_set_occlusion_enabled(enabled as bool32_t)
        }
    }

    fn world_get_occlusion_enabled(&self) -> bool {
        unsafe {
            stereokit_sys::world_get_occlusion_enabled() != 0
        }
    }

    fn world_set_raycast_enabled(&self, enabled: bool) {
        unsafe {
            stereokit_sys::world_set_raycast_enabled(enabled as bool32_t)
        }
    }

    fn world_get_raycast_enabled(&self) -> bool {
        unsafe {
            stereokit_sys::world_get_raycast_enabled() != 0
        }
    }

    fn world_set_occlusion_material(&self, material: impl AsRef<Material>) {
        unsafe {
            stereokit_sys::world_set_occlusion_material(material.as_ref().0.as_ptr())
        }
    }

    fn world_get_occlusion_material(&self) -> Material {
        Material(NonNull::new(
            unsafe { stereokit_sys::world_get_occlusion_material() }
        ).unwrap())
    }

    fn world_set_refresh_type(&self, refresh_type: WorldRefresh) {
        unsafe {
            stereokit_sys::world_set_refresh_type(refresh_type as world_refresh_)
        }
    }

    fn world_get_refresh_type(&self) -> WorldRefresh {
        unsafe {
            std::mem::transmute(stereokit_sys::world_get_refresh_type())
        }
    }

    fn world_set_refresh_radius(&self, radius_meters: f32) {
        unsafe {
            stereokit_sys::world_set_refresh_radius(radius_meters)
        }
    }

    fn world_get_refresh_radius(&self) -> f32 {
        unsafe {
            stereokit_sys::world_get_refresh_radius()
        }
    }

    fn world_get_refresh_interval(&self) -> f32 {
        unsafe {
            stereokit_sys::world_get_refresh_interval()
        }
    }

    fn backend_xr_get_type(&self) -> BackendXrType {
        unsafe {
            std::mem::transmute(stereokit_sys::backend_xr_get_type())
        }
    }

    fn backend_openxr_get_instance(&self) -> OpenXrHandle {
        unsafe {
            stereokit_sys::backend_openxr_get_instance()
        }
    }

    fn backend_openxr_get_session(&self) -> OpenXrHandle {
        unsafe {
            stereokit_sys::backend_openxr_get_session()
        }
    }

    fn backend_openxr_get_system_id(&self) -> OpenXrHandle {
        unsafe {
            stereokit_sys::backend_openxr_get_system_id()
        }
    }

    fn backend_openxr_get_space(&self) -> OpenXrHandle {
        unsafe {
            stereokit_sys::backend_openxr_get_space()
        }
    }
    ...
    fn backend_openxr_get_function<S: AsRef<str>>(
        ...
        unsafe {
            stereokit_sys::backend_openxr_get_function(function_name.as_ptr())
        }
    }
    ...
    fn backend_openxr_ext_enabled<S: AsRef<str>>(&self, extension_name: S, ) -> bool {
        ...
        unsafe {
            stereokit_sys::backend_openxr_ext_enabled(extension_name.as_ptr()) != 0
        }
    }
    ...
    fn backend_openxr_ext_request<S: AsRef<str>>(&self, extension_name: S) {
        let extension_name = CString::new(extension_name.as_ref()).unwrap();
        unsafe {
            stereokit_sys::backend_openxr_ext_request(extension_name.as_ptr())
        }
    }

    fn backend_openxr_use_minimum_exts(&self, use_minimum_exts: bool) {
        unsafe {
            stereokit_sys::backend_openxr_use_minimum_exts(use_minimum_exts as bool32_t)
        }
    }
    ...
    fn log_diag<S: AsRef<str>>(&self, text: S) {
        ...
        unsafe {
            stereokit_sys::log_diag(text.as_ptr())
        }
    }
    ...
    fn log_info<S: AsRef<str>>(&self, text: S) {
        let text = CString::new(text.as_ref()).unwrap();
        unsafe {
            stereokit_sys::log_info(text.as_ptr())
        }
    }
    ...
    fn log_warn<S: AsRef<str>>(&self, text: S) {
        let text = CString::new(text.as_ref()).unwrap();
        unsafe {
            stereokit_sys::log_warn(text.as_ptr())
        }
    }
    ...
    fn log_err<S: AsRef<str>>(&self, text: S) {
        let text = CString::new(text.as_ref()).unwrap();
        unsafe {
            stereokit_sys::log_err(text.as_ptr())
        }
    }
    ...
    fn log_write<S: AsRef<str>>(&self, level: LogLevel, text: S) {
        let text = CString::new(text.as_ref()).unwrap();
        unsafe {
            stereokit_sys::log_write(level as log_,text.as_ptr())
        }
    }

    fn log_set_filter(&self, level: LogLevel) {
        unsafe {
            stereokit_sys::log_set_filter(level as log_)
        }
    }

    fn log_set_colors(&self, colors: LogColors) {
        unsafe {
            stereokit_sys::log_set_colors(colors as log_colors_)
        }
    }
    ...
    fn assets_releaseref_threadsafe(&self, asset: Asset) {
        unsafe {
            stereokit_sys::assets_releaseref_threadsafe(asset.0.as_ptr())
        }
    }

    fn assets_current_task(&self) -> i32 {
        unsafe {
            stereokit_sys::assets_current_task()
        }
    }

    fn assets_total_tasks(&self) -> i32 {
        unsafe {
            stereokit_sys::assets_total_tasks()
        }
    }

    fn assets_current_task_priority(&self) -> i32 {
        unsafe {
            stereokit_sys::assets_current_task_priority()
        }
    }

    fn assets_block_for_priority(&self, priority: i32) {
        unsafe {
            stereokit_sys::assets_block_for_priority(priority)
        }
    }

    fn assets_count(&self) -> i32 {
        unsafe {
            stereokit_sys::assets_count()
        }
    }
    ...
    fn assets_get_type(&self, index: i32) -> AssetType {
        unsafe {
            std::mem::transmute(stereokit_sys::assets_get_type(index))
        }
    }
    ...
    fn asset_set_id(&self, asset: impl AsRef<Asset>, id: impl AsRef<str>) {
        let id = CString::new(id.as_ref()).unwrap();
        unsafe {
            stereokit_sys::asset_set_id(asset.as_ref().0.as_ptr(), id.as_ptr())
        }
    }
    ...
    fn asset_get_id(&self, asset: impl AsRef<Asset>) -> &str {
        unsafe {
            CStr::from_ptr(
                stereokit_sys::asset_get_id(asset.as_ref().0.as_ptr())
            )
        }.to_str().unwrap()
    }
...
pub fn ray_intersect_plane<V3: Into<Vec3>>(
    ...
    let mut out_t = 0.0;
    match unsafe {
        stereokit_sys::ray_intersect_plane(ray.into(), plane_pt.into().into(), plane_normal.into().into(), &mut out_t) != 0
    } {
        true => Some(out_t),
        false => None,
    }
...
pub fn ray_from_mouse(screen_pixel_pos: impl Into<Vec2>) -> Option<Ray> {
    ...
    match unsafe {
        stereokit_sys::ray_from_mouse(screen_pixel_pos.into().into(), &mut ray) != 0
    } {
        true => Some(ray.into()),
        false => None,
    }
...
pub fn plane_from_points<V3: Into<Vec3>>(
    p1: V3,
    p2: V3,
    p3: V3
) -> Plane {
...
pub fn plane_from_ray(ray: Ray) -> Plane {
    unsafe {
        stereokit_sys::plane_from_ray(ray.into())
    }.into()
}
...
pub fn plane_ray_intersect(
    plane: Plane,
    ray: Ray
) -> Option<Vec3> {
    let mut point = Vec3::default().into();
    match unsafe {
        stereokit_sys::plane_ray_intersect(plane.into(), ray.into(), &mut point) != 0
    } {
...
pub fn plane_line_intersect<V3: Into<Vec3>>(
    plane: Plane,
    p1: V3,
    p2: V3
) -> Option<Vec3> {
    let mut point = Vec3::default().into();
    match unsafe {
        stereokit_sys::plane_line_intersect(plane.into(), p1.into().into(), p2.into().into(), &mut point) != 0
    } {
        true => Some(point.into()),
        false => None,
    }
...
pub fn plane_point_closest<V3: Into<Vec3>>(
    plane: Plane,
    pt: V3,
) -> Vec3 {
    unsafe {
        stereokit_sys::plane_point_closest(plane.into(), pt.into().into())
    }.into()
}
...
pub fn sphere_ray_intersect(
    sphere: Sphere,
    ray: Ray,
) -> Option<Vec3> {
    let mut pt = Vec3::default().into();
    match unsafe {
        stereokit_sys::sphere_ray_intersect(sphere.into(), ray.into(), &mut pt) != 0
    } {
        true => Some(pt.into()),
        false => None,
    }
...
pub fn sphere_point_contains<V3: Into<Vec3>>(
    sphere: Sphere,
    pt: V3,
) -> bool {
    unsafe {
        stereokit_sys::sphere_point_contains(sphere.into(), pt.into().into()) != 0
    }
}

pub fn bounds_ray_intersect(
    bounds: Bounds,
    ray: Ray
) -> Option<Vec3> {
    let mut pt = Vec3::default().into();
    match unsafe {
        stereokit_sys::bounds_ray_intersect(bounds.into(), ray.into(), &mut pt) != 0
    } {
        true => Some(pt.into()),
        false => None,
    }
...
pub fn bounds_point_contains<V3: Into<Vec3>>(
    bounds: Bounds,
    pt: V3
) -> bool {
    unsafe {
        stereokit_sys::bounds_point_contains(bounds.into(), pt.into().into()) != 0
    }
}

pub fn bounds_line_contains<V3: Into<Vec3>>(
    bounds: Bounds,
    pt1: V3,
    pt2: V3,
) -> bool {
    unsafe {
        stereokit_sys::bounds_line_contains(bounds.into(), pt1.into().into(), pt2.into().into()) != 0
    }
}
...
pub fn bounds_capsule_contains<V3: Into<Vec3>>(
    ...
    unsafe {
        stereokit_sys::bounds_capsule_contains(bounds.into(), pt1.into().into(), pt2.into().into(), radius) != 0
    }
}

pub fn bounds_grow_to_fit_pt<V3: Into<Vec3>>(
    bounds: Bounds,
    pt: V3,
) -> Bounds {
    unsafe {
        stereokit_sys::bounds_grow_to_fit_pt(bounds.into(), pt.into().into()).into()
    }
}
...
pub fn bounds_transform(
    bounds: Bounds,
    transform: impl Into<Mat4>
) -> Bounds {
    unsafe {
        stereokit_sys::bounds_transform(bounds.into(), transform.into().into())
            .into()
    }
}

pub fn ray_point_closest(
    ray: Ray,
    pt: impl Into<Vec3>,
) -> Vec3 {
    unsafe {
        stereokit_sys::ray_point_closest(
            ray.into(),
            pt.into().into()
        ).into()
    }
...
pub fn color_to_hsv(color: Color128) -> Vec3 {
    unsafe {
        stereokit_sys::color_to_hsv(&color).into()
    }
}
...
pub fn color_lab(l: f32, a: f32, b: f32, transparency: f32) -> Color128 {
    unsafe {
        stereokit_sys::color_lab(l, a, b, transparency)
    }
}
...
pub fn color_to_lab(color: Color128) -> Vec3 {
    unsafe {
        stereokit_sys::color_to_lab(&color)
    }.into()
}
...
pub fn color_to_linear(srgb_gamma_correct: Color128) -> Color128 {
    unsafe {
        stereokit_sys::color_to_linear(srgb_gamma_correct)
    }
}
...
pub fn color_to_gamma(srgb_linear: Color128) -> Color128 {
    unsafe {
        stereokit_sys::color_to_gamma(srgb_linear)
    }
}

/// Creates a new, completely empty gradient.
pub fn gradient_create() -> Gradient {
    Gradient(NonNull::new(unsafe {
        stereokit_sys::gradient_create()
    }).unwrap())
}
...
pub fn gradient_create_keys(keys: &[GradientKey]) -> Gradient {
    Gradient(NonNull::new(
        unsafe {
            stereokit_sys::gradient_create_keys(keys.iter().map(|a| a.clone().into()).collect::<Vec<_>>().as_ptr(), keys.len() as i32)
        }
    ).unwrap())
}
...
pub fn gradient_add(gradient: impl AsRef<Gradient>, color_linear: Color128, position: f32) {
    unsafe {
        stereokit_sys::gradient_add(gradient.as_ref().0.as_ptr(), color_linear, position)
    }
}
...
pub fn gradient_set(gradient: impl AsRef<Gradient>, index: i32, color_linear: Color128, position: f32) {
    unsafe {
...
pub fn gradient_remove(gradient: impl AsRef<Gradient>, index: i32) {
    unsafe {
        stereokit_sys::gradient_remove(gradient.as_ref().0.as_ptr(), index)
    }
}
...
pub fn gradient_count(gradient: impl AsRef<Gradient>) -> i32 {
    unsafe {
        stereokit_sys::gradient_count(gradient.as_ref().0.as_ptr())
    }
}
...
pub fn gradient_get(gradient: impl AsRef<Gradient>, at: f32) -> Color128 {
    unsafe {
        stereokit_sys::gradient_get(gradient.as_ref().0.as_ptr(), at)
    }
}
...
pub fn gradient_get32(gradient: impl AsRef<Gradient>, at: f32) -> Color32 {
    unsafe {
        stereokit_sys::gradient_get32(gradient.as_ref().0.as_ptr(), at)
    }
}
...
pub fn gradient_release(gradient: &mut Gradient) {
    unsafe {
        stereokit_sys::gradient_release(gradient.0.as_ptr())
    }
}
...
pub fn sh_create(lights: &[ShLight]) -> SphericalHarmonics {
    unsafe {
        stereokit_sys::sh_create(lights.iter().map(|a| a.clone().into()).collect::<Vec<_>>().as_ptr(), lights.len() as i32)
    }.into()
}
...
pub fn sh_brightness(ref_harmonics: &mut SphericalHarmonics, scale: f32) {
    unsafe {
        stereokit_sys::sh_brightness(std::mem::transmute(ref_harmonics), scale)
    }
}
...
pub fn sh_add(ref_harmonics: &mut SphericalHarmonics, light_dir: Vec3, light_color: Vec3) {
    unsafe {
        stereokit_sys::sh_add(std::mem::transmute(ref_harmonics), light_dir.into(), light_color.into())
    }
}
...
pub fn sh_lookup(harmonics: &SphericalHarmonics, normal: Vec3) -> Color128 {
    unsafe {
        stereokit_sys::sh_lookup(&(*harmonics).into(), normal.into())
    }
}
...
pub fn sh_dominant_dir(harmonics: &SphericalHarmonics) -> Vec3 {
    unsafe {
        stereokit_sys::sh_dominant_dir(&(*harmonics).into())
    }.into()
}
...
fn mesh_release(mesh: &mut Mesh) {
    unsafe {
        stereokit_sys::mesh_release(mesh.0.as_ptr())
    }
}

fn font_release(font: &mut Font) {
    unsafe {
        stereokit_sys::font_release(font.0.as_ptr())
    }
}

fn shader_release(shader: &mut Shader) {
    unsafe {
        stereokit_sys::shader_release(shader.0.as_ptr())
    }
}

fn material_release(material: &mut Material) {
    unsafe {
        stereokit_sys::material_release(material.0.as_ptr())
    }
}

fn material_buffer_release(material_buffer: &mut MaterialBuffer) {
    unsafe {
        stereokit_sys::material_buffer_release(material_buffer.0.as_ptr())
    }
}

fn model_release(model: &mut Model) {
    unsafe {
        stereokit_sys::model_release(model.0.as_ptr())
    }
}

fn sprite_release(sprite: &mut Sprite) {
    unsafe {
        stereokit_sys::sprite_release(sprite.0.as_ptr())
    }
}

fn sound_release(sound: &mut Sound) {
    unsafe {
        stereokit_sys::sound_release(sound.0.as_ptr())
    }
}
...
impl WindowContext {
    pub fn push_text_style(&self, style: TextStyle) {
        unsafe {
            stereokit_sys::ui_push_text_style(style.0)
        }
    }
    ...
    pub fn push_preserve_keyboard(&self, preserve_keyboard: bool) {
        unsafe {
            stereokit_sys::ui_push_preserve_keyboard(preserve_keyboard as bool32_t)
        }
    }
    ...
    pub fn preserve_keyboard(&self, preserve_keyboard: bool, content_closure: impl FnOnce(&WindowContext)) {
        self.push_preserve_keyboard(preserve_keyboard);
        content_closure(&self);
        self.pop_preserve_keyboard();
    }
    pub fn push_surface(&self, surface_pose: Pose, layout_state: impl Into<Vec3>, layout_dimensions: impl Into<Vec2>) {
        unsafe {
            stereokit_sys::ui_push_surface(surface_pose.into(), layout_state.into().into(), layout_dimensions.into().into())
        }
    }
    pub fn pop_surface(&self) {
        unsafe {
            stereokit_sys::ui_pop_surface()
        }
    }
    pub fn surface(&self, surface_pose: Pose, layout_state: impl Into<Vec3>, layout_dimensions: impl Into<Vec2>, content_closure: impl FnOnce(&WindowContext)) {
    ...
    pub fn push_id(&self, id: impl AsRef<str>) -> u64 {
        let id = CString::new(id.as_ref()).unwrap();
        unsafe {
            stereokit_sys::ui_push_id(id.as_ptr())
        }
    }
    pub fn push_idi(&self, id: i32) -> u64 {
        unsafe {
            stereokit_sys::ui_push_idi(id)
        }
    }
    pub fn pop_id(&self) {
        unsafe {
            stereokit_sys::ui_pop_id()
        }
    }
